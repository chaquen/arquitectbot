...
soportando múltiples destinos separados por punto y coma.
"""
import os
import sys
...
def test_import_edges_from_csv_multiple_targets():
    """Test importing edges from CSV directly to Neo4j with multiple targets support"""
    ...
    logger.info(f"Se leyeron {len(edges)} relaciones del CSV.")
                ...
                try:
                    ...
                    if exists_check and exists_check['exists']:
                        ...
                        if update_result:
                            ...
                            errors += 1
                            logger.error(f"❌ No se pudo actualizar la relación: {source} -> {target}")
                        continue
...
if __name__ == "__main__":
    success = test_import_edges_from_csv_multiple_targets()
    if success:
