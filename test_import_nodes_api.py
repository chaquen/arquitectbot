"""
Script para probar la importación de nodos a través de la API Flask.
"""
import requests
import os
import logging
...
def test_import_nodes_via_api():
    """Prueba la importación de nodos mediante el endpoint de la API Flask"""
    # URL de la API
    base_url = "http://localhost:5000"
    
    # Archivo CSV de nodos
    ...
    # Importar nodos mediante la API
    try:
        logger.info(f"Importando nodos desde {csv_path}")
        with open(csv_path, 'rb') as file:
            files = {'file': (os.path.basename(csv_path), file, 'text/csv')}
            response = requests.post(
                f"{base_url}/import-nodes-components",
                files=files
            )
        ...
        if result.get('created', 0) > 0:
            ...
            logger.warning("⚠️ No se importaron nodos (posiblemente ya existían).")
            return True
...
if __name__ == "__main__":
    success = test_import_nodes_via_api()
    if success:
        print("\n✅ Prueba de importación de nodos vía API exitosa.")
    else:
